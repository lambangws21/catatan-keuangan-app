...
import { Loader2, Plus, Stethoscope, Calendar, User, Hospital, DollarSign, FileSignature, Edit } from "lucide-react";
...
export default function OperationForm({ onFormSubmit, initialData }: OperationFormProps) {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      await onFormSubmit();
      toast.success(`Data operasi berhasil ${isEditMode ? 'diperbarui' : 'disimpan'}!`);
      resetFormAndClose();
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[625px] bg-gray-800/80 backdrop-blur-md border-gray-700 text-white">
        <DialogHeader>
          <DialogTitle className="text-cyan-400">{isEditMode ? 'Edit' : 'Input'} Data Operasi</DialogTitle>
          <p className="text-sm text-gray-400">Masukkan detail tindakan operasi.</p>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="grid gap-4 py-4 max-h-[80vh] overflow-y-auto pr-6">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                        ...
                        <div className="relative"><Calendar className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" /><Input id="date" name="date" type="date" value={formData.date} onChange={handleChange} required className="pl-10"/></div>
                    </div>
                    <div className="space-y-2">
                        <Label htmlFor="dokter">Dokter</Label>
                        <div className="relative"><User className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" /><Input id="dokter" name="dokter" value={formData.dokter} onChange={handleChange} placeholder="Nama Dokter" required className="pl-10"/></div>
                    </div>
                </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                <div className="space-y-2">
                    ...
                    <div className="relative"><Stethoscope className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" /><Input id="tindakanOperasi" name="tindakanOperasi" value={formData.tindakanOperasi} onChange={handleChange} placeholder="Contoh: Operasi Caesar" required className="pl-10"/></div>
                </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                <div className="space-y-2">
                    <Label htmlFor="rumahSakit">Rumah Sakit</Label>
                    <div className="relative"><Hospital className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" /><Input id="rumahSakit" name="rumahSakit" value={formData.rumahSakit} onChange={handleChange} placeholder="Nama Rumah Sakit" required className="pl-10"/></div>
                </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative"><FileSignature className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" /><Input id="klaim" name="klaim" value={formData.klaim} onChange={handleChange} placeholder="Contoh: BPJS" required className="pl-10"/></div>
                    </div>
                </div>
            </motion.div>
